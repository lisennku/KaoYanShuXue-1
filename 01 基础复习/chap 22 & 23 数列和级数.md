...
4. 交错级数判别法
    ...
    > 交错级数判别法，指的是
    >
    > 如果数列$\{a_n\}$是正负交错，各项的绝对值递减，且趋近于$0$，则级数$\displaystyle \sum_{n=1}^{\infty}a_n$收敛
...
5. 应用积分判别法
    ...
    > 若对连续递减正函数，有$a_n = f(n)$，则$\int_1^{\infty}f(x)dx \text{和}\sum_{n = 1}^{\infty}a_n$同时发散或收敛
